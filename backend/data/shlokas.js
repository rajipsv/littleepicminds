const CHAPTER_1_SHLOKAS = {
    "1.1": {
        sanskrit: "धृतराष्ट्र उवाच।\nधर्मक्षेत्रे कुरुक्षेत्रे समवेता युयुत्सवः।\nमामकाः पाण्डवाश्चैव किमकुर्वत सञ्जय॥",
        transliteration: "dhṛtarāṣṭra uvāca\ndharmakṣetre kurukṣetre samavetā yuyutsavaḥ\nmāmakāḥ pāṇḍavāścaiva kim akurvata sañjaya",
        en: {
            meaning: "Dhritarashtra said: O Sanjaya, gathered on the holy field of Kurukshetra, eager to fight, what did my sons and the sons of Pandu do?",
            childMeaning: "A king asks: 'Tell me, what is happening on the big playground where the two teams have gathered for the match?'",
            activity: "Draw a picture of two teams getting ready for a friendly game!"
        },
        te: {
            meaning: "ధృతరాష్ట్రుడు ఇలా అన్నాడు: ఓ సంజయా, పుణ్యక్షేత్రమైన కురుక్షేత్రంలో యుద్ధం చేయడానికి సిద్ధపడి చేరిన నా కుమారులు మరియు పాండు కుమారులు ఏమి చేశారు?",
            childMeaning: "ఒక రాజు అడుగుతున్నాడు: 'రెండు జట్లు పోటీ కోసం సిద్ధమైన ఆ మైదానంలో ఏమి జరుగుతుందో నాకు చెప్పు?'",
            activity: "రెండు జట్లు ఆడుకోవడానికి సిద్ధమవుతున్న చిత్రాన్ని గీయండి!"
        },
        exercises: {
            seeds: { question: "How do the teams feel about the game?", question_te: "జట్టు సభ్యులు ఆట గురించి ఎలా భావిస్తున్నారు?", options: ["Eager/Excited (🤩)", "Sad (😢)", "Sleepy (😴)"], correct: 0 },
            seekers: { question: "Why did the two groups gather at Kurukshetra?", question_te: "కురుక్షేత్రంలో రెండు జట్లు ఎందుకు చేరాయి?", options: ["Birthday party", "Prepare for battle", "Plant a garden"], correct: 1 },
            warriors: { question: "Significance of 'Dharmakshetra'?", question_te: "ధర్మక్షేత్రం అంటే ఏమిటి?", options: ["Playground", "Field of tested righteousness", "Name of a city"], correct: 1 }
        },
        telugu_script: "ధృతరాష్ట్ర ఉవాచ |\nధర్మక్షేత్రే కురుక్షేత్రే సమవేతా యుయుత్సవః |\nమామకాః పాండవాశ్చైవ కిమకుర్వత సంజయ ||",
        lineBreakdown: [
            { sanskrit: "धर्मक्षेत्रे कुरुक्षेत्रे", word: "dharmakṣetre kurukṣetre", sanskrit_te: "ధర్మక్షేత్రే కురుక్షేత్రే", en: "In the holy field of Kurukshetra", te: "ధర్మక్షేత్రమైన కురుక్షేత్రంలో" },
            { sanskrit: "समवेता युयुत्सवः", word: "samavetā yuyutsavaḥ", sanskrit_te: "సమవేతా యుయుత్सवః", en: "gathered together, desiring to fight", te: "యుద్ధం చేయడానికి సిద్ధపడి చేరిన" },
            { sanskrit: "मामकाः पाण्डवाश्चैव", word: "māmakāḥ pāṇḍavāścaiva", sanskrit_te: "మామకాః పాండవాశ్చైవ", en: "my sons and the sons of Pandu", te: "నా కుమారులు మరియు పాండు కుమారులు" },
            { sanskrit: "किमकुर्वत सञ्जय", word: "kim akurvata sañjaya", sanskrit_te: "కిమకుర్వత సంజయ", en: "what did they do, O Sanjaya?", te: "ఏమి చేశారు, ఓ సంజయా?" }
        ]
    },
    "1.2": {
        sanskrit: "सञ्जय उवाच।\nदृष्ट्वा तु पाण्डवानीकं व्यूढं दुर्योधनस्तदा।\nआचार्यमुपसङ्गम्य राजा वचनमब्रवीत्॥",
        transliteration: "sañjaya uvāca\ndṛṣṭvā tu pāṇḍavānīkaṁ vyūḍhaṁ duryodhanas tadā\nācāryam upasaṅgamya rājā vacanam abravīt",
        en: {
            meaning: "Sanjaya said: After looking over the army gathered by the sons of Pandu, King Duryodhana went to his teacher and spoke these words.",
            childMeaning: "Duryodhana saw the other team ready and went to his coach to talk about their strategy.",
            activity: "Who is your favorite teacher? Write one thing they taught you."
        },
        te: {
            meaning: "సంజయుడు ఇలా అన్నాడు: పాండవుల సైన్యాన్ని చూసిన దుర్యోధన రాజు, తన ద్రోణాచార్య గురువు దగ్గరకు వెళ్ళి ఇలా అన్నాడు.",
            childMeaning: "దుర్యోధనుడు అవతలి జట్టు సిద్ధంగా ఉండటం చూసి, తన కోచ్ దగ్గరకు వెళ్ళి మాట్లాడాడు.",
            activity: "మీకు ఇష్టమైన టీచర్ ఎవరు? వారు నేర్పిన ఒక మంచి విషయం రాయండి."
        },
        exercises: {
            seeds: { question: "Who did Duryodhana go to see?", question_te: "దుర్యోధనుడు ఎవరిని కలవడానికి వెళ్ళాడు?", options: ["Friend (🤝)", "Coach/Teacher (👨🏫)", "Pet (🐶)"], correct: 1 },
            seekers: { question: "Who was Duryodhana's important teacher?", question_te: "దుర్యోధనుడి ముఖ్యమైన గురువు ఎవరు?", options: ["Krishna", "Dronacharya", "Dhritarashtra"], correct: 1 },
            warriors: { question: "Duryodhana's action indicates:", question_te: "దుర్యోధనుడు గురువు దగ్గరకు వెళ్ళడం దేనిని సూచిస్తుంది?", options: ["Respect", "Anxiety and need for reassurance", "He wanted to quit"], correct: 1 }
        },
        telugu_script: "సంజయ ఉవాచ |\nదృష్ట్వా తు పాండవానీకం వ్యూఢం దుర్యోధనస్తదా |\nఆచార్యముపసంగమ్య రాజా వచనమబ్రవీత్ ||",
        lineBreakdown: [
            { sanskrit: "दृष्ट्वा तु पाण्डवानीकं", word: "dṛṣṭvā tu pāṇḍavānīkaṁ", sanskrit_te: "దృష్ట్వా తు పాండవానీకం", en: "after seeing the army of the Pandavas", te: "పాండవుల సైన్యాన్ని చూసిన తరువాత" },
            { sanskrit: "व्यूटं दुर्योधनस्तदा", word: "vyūḍhaṁ duryodhanas tadā", sanskrit_te: "వ్యూఢం దుర్యోధనస్తదా", en: "arrayed in battle formation, Duryodhana", te: "యుద్ధ శ్రేణిలో ఉన్న దుర్యోధనుడు" },
            { sanskrit: "आचार्यमुपसङ्गम्य", word: "ācāryam upasaṅgamya", sanskrit_te: "ఆచార్యముపసంగమ్య", en: "approached his teacher", te: "తన గురువు దగ్గరకు వెళ్ళి" },
            { sanskrit: "राजा वचनमब्रवीत्", word: "rājā vacanam abravīt", sanskrit_te: "రాజా వచనమబ్రవీత్", en: "the king spoke these words", te: "రాజు ఈ మాటలు పలికాడు" }
        ]
    },
    "1.3": {
        sanskrit: "पश्यैतां पाण्डुपुत्राणामाचार्य महतीं चमूम्।\nव्यूढां द्रुपदपुत्रेण तవ शिष्येण धीमता॥",
        transliteration: "paśyaitāṁ pāṇḍu-putrāṇām ācārya mahatīṁ camūm\nvyūḍhāṁ drupada-putreṇa tava śiṣyeṇa dhīmatā",
        en: {
            meaning: "O teacher, behold the great army of the sons of Pandu, arranged by your intelligent disciple, the son of Drupada.",
            childMeaning: "Duryodhana said: 'Look, Teacher! See how smart the other team is. They arranged their players so well!'",
            activity: "Think of a smart friend. What do you admire about them?"
        },
        te: {
            meaning: "ఓ ఆచార్యా! మీ బుద్ధిమంతుడైన శిష్యుడు, దృపద పుత్రుడిచే వ్యూహరచన చేయబడిన ఈ గొప్ప పాండవ సైన్యాన్ని చూడండి.",
            childMeaning: "దుర్యోధనుడు ఇలా అన్నాడు: 'గురువు గారు! అవతలి జట్టు ఎంత తెలివిగా ఉందో చూడండి. వారు తమ ఆటగాళ్లను ఎంత చక్కగా అమర్చారో!'",
            activity: "ఒక తెలివైన స్నేహితుడి గురించి ఆలోచించండి. వారిలో మీకు నచ్చే విషయం ఏమిటి?"
        },
        exercises: {
            seeds: { question: "The other team was...", question_te: "అవతలి జట్టు ఎలా ఉంది?", options: ["Smart (🧠)", "Sleeping (😴)", "Eating (🍎)"], correct: 0 },
            seekers: { question: "Who arranged the Pandava army?", question_te: "పాండవ సైన్యాన్ని ఎవరు అమర్చారు?", options: ["Arjuna", "Dhrishtadyumna", "Krishna"], correct: 1 },
            warriors: { question: "Why mention Drona's disciple?", question_te: "ద్రోణాచార్యుడి శిష్యుడి గురించి ఎందుకు ప్రస్తావించారు?", options: ["Praise", "Subtle taunt", "Autograph"], correct: 1 }
        },
        telugu_script: "పశ్యైతాం పాండుపుత్రాణామాచార్య మహతీం చమూమ్ |\nవ్యూఢాం దృపదపుత్రేణ తవ శిష్యేణ ధీమతా ||",
        lineBreakdown: [
            { sanskrit: "पश्यैतां पाण्डुपुत्राणां", word: "paśyaitāṁ pāṇḍu-putrāṇām", sanskrit_te: "పశ్యైతాం పాండుపుత్రాణాం", en: "behold this army of the sons of Pandu", te: "ఈ పాండవుల సైన్యాన్ని చూడండి" },
            { sanskrit: "आचार्य महतीं चमूम्", word: "ācārya mahatīṁ camūm", sanskrit_te: "ఆచార్య మహతీం చమూమ్", en: "O teacher, this great army", te: "ఓ ఆచార్యా! ఈ గొప్ప సైన్యం" },
            { sanskrit: "व्यूटं द्रुपदपुत्रेण", word: "vyūḍhāṁ drupada-putreṇa", sanskrit_te: "వ్యూఢాం దృపదపుత్రేణ", en: "arranged by the son of Drupada", te: "దృపద పుత్రుడిచే వ్యూహరచన చేయబడింది" },
            { sanskrit: "తవ శిష్యేణ ధీమతా", word: "tava śiṣyeṇa dhīmatā", sanskrit_te: "తవ శిష్యేణ ధీమతా", en: "by your intelligent disciple", te: "మీ బుద్ధిమంతుడైన శిష్యుడిచే" }
        ]
    },
    "1.4": {
        sanskrit: "अत्र शूरा महेष्वासा भीमार्जुनसमा युधि।\nयुयुधानो विराटश्च द्रुपदश्च महारथः॥",
        transliteration: "atra śūrā maheṣv-āsā bhīmārjuna-samā yudhi\nyuyudhāno virāṭaś ca drupadaś ca mahā-rathaḥ",
        en: {
            meaning: "Here in this army are many heroic bowmen equal in fighting to Bhima and Arjuna: great fighters like Yuyudhana, Virata and Drupada.",
            childMeaning: "Duryodhana said: 'Look at their star players! They have heroes just as strong as Bhima and Arjuna. They are all great warriors!'",
            activity: "If you were forming a 'Dream Team' for a game, who are the first 3 friends you would pick? Why?"
        },
        te: {
            meaning: "ఇక్కడ యుద్ధంలో భీమార్జునులతో సమానమైన గొప్ప వీరులు, విలుకారులు ఉన్నారు. యుయుధానుడు, విరాటుడు మరియు మహారథుడైన దృపదుడు వంటి వారు ఉన్నారు.",
            childMeaning: "దుర్యోధనుడు ఇలా అన్నాడు: 'వారి స్టార్ ప్లేయర్లను చూడండి! భీమార్జునులంత బలవంతులైన వీరులు వారి దగ్గర ఉన్నారు.'",
            activity: "మీరు ఒక 'డ్రీమ్ టీమ్' తయారు చేస్తే, మొదట ఏ ముగ్గురు స్నేహితులను ఎంచుకుంటారు? ఎందుకు?"
        },
        exercises: {
            seeds: { question: "Who are the two famous heroes mentioned here?", question_te: "ఇక్కడ పేర్కొన్న ఇద్దరు ప్రసిద్ధ వీరులు ఎవరు?", options: ["Bhima & Arjuna (🏹)", "Two Kittens (🐱)", "Sun & Moon (☀️)"], correct: 0 },
            seekers: { question: "Duryodhana compares the other warriors to whom?", question_te: "దుర్యోధనుడు అవతలి వీరులను ఎవరితో పోల్చాడు?", options: ["Himself", "Bhima and Arjuna", "Krishna"], correct: 1 },
            warriors: { question: "What does the term 'Maha-rathah' mean?", question_te: "'మహారథుడు' అంటే ఎవరు?", options: ["A small cart", "A great warrior who can fight thousands alone", "A fast runner"], correct: 1 }
        },
        telugu_script: "అత్ర శూరా మహేష్వాసా భీమార్జునసమా యుధి |\nయుయుధానో విరాటశ్చ దృపదశ్చ మహారథః ||",
        lineBreakdown: [
            { sanskrit: "atra śūrā maheṣv-āsā", sanskrit_te: "అత్ర శూరా మహేష్వాసా", en: "here are heroes, great bowmen", te: "ఇక్కడ గొప్ప విలుకారులు, వీరులు ఉన్నారు" },
            { sanskrit: "bhīmārjuna-samā yudhi", sanskrit_te: "భీమార్జునసమా యుధి", en: "equal to Bhima and Arjuna in battle", te: "యుద్ధంలో భీమార్జునులతో సమానమైనవారు" }
        ]
    },
    "2.6": {
        sanskrit: "न चैतद्विद्मः कतरन्नो गरीयो यद्वा जयेम यदि वा नो जयेयुः।\nयानेव हत्वा न जिजीविषामस्तेऽवस्थिताः प्रमुखे धार्तराष्ट्राः॥",
        transliteration: "na caitad vidmaḥ kataran no garīyo yad vā jayema yadi vā no jayeyuḥ\nyān eva hatvā na jijīviṣāmas te ’vasthitāḥ pramukhe dhārtarāṣṭrāḥ",
        en: {
            meaning: "Nor do we know which is better—conquering them or being conquered by them. If we killed the sons of Dhritarashtra, we should not care to live.",
            childMeaning: "Arjuna said: 'I don't even know if it's better to win or lose. If I hurt my cousins, I won't even feel like living anymore.'",
            activity: "Think of a time you were very confused. How did you decide what to do?"
        },
        te: {
            meaning: "మనకు ఏది మేలో తెలియడం లేదు - మనం గెలవడమా లేక వారు మనల్ని గెలవడమా? ఎవరిని చంపి మనం బ్రతకాలని అనుకోవడం లేదో, ఆ ధృతరాష్ట్రుని కుమారులే మన ముందు నిలబడి ఉన్నారు.",
            childMeaning: "అర్జునుడు అన్నాడు: 'గెలవడం మంచిదో, ఓడిపోవడం మంచిదో నాకు తెలియడం లేదు. నా అన్నదమ్ములను బాధపెట్టి నేను ఎలా సంతోషంగా ఉండగలను?'",
            activity: "నీకు ఎప్పుడైనా చాలా అయోమయంగా అనిపించిందా? అప్పుడు నువ్వు ఏం చేశావు?"
        },
        exercises: {
            seeds: { question: "Was Arjuna sure about what to do?", question_te: "అర్జునుడు ఏమి చేయాలో ఖచ్చితంగా తెలుసా?", options: ["Yes", "No, very confused (😵)", "Maybe"], correct: 1 },
            seekers: { question: "Who was Arjuna worried about hurting?", question_te: "అర్జునుడు ఎవరిని బాధపెడతానని భయపడ్డాడు?", options: ["Strangers", "The sons of Dhritarashtra (His family)", "Monsters"], correct: 1 },
            warriors: { question: "Arjuna's dilemma in 2.6?", question_te: "2.6 లో అర్జునుడి ప్రధాన సమస్య ఏమిటి?", options: ["Fear of losing", "Confusion over whether victory is worth the cost", "He was tired"], correct: 1 }
        },
        telugu_script: "న చైతద్విద్మః కతరన్నో గరీయో యద్వా జయేమ యది వా నో జయేయుః |\nయానేవ హత్వా న జిజీవిషామస్తేఽవస్థితాః ప్రముఖే ధార్తరాష్ట్రాః ||",
        lineBreakdown: [
            { sanskrit: "na caitad vidmaḥ kataran no garīyo", sanskrit_te: "న చైతద్విద్మః కతరన్నో గరీయో", en: "nor do we know which is better for us", te: "మనకు ఏది మేలో మనకు తెలియదు" },
            { sanskrit: "yad vā jayema yadi vā no jayeyuḥ", sanskrit_te: "యద్వా జయేమ యది వా నో జయేయుః", en: "whether we conquer or they conquer us", te: "మనం గెలవడమా లేక వారు మనల్ని గెలవడమా" },
            { sanskrit: "yān eva hatvā na jijīviṣāmas", sanskrit_te: "యానేవ హత్వా న జిజీవిషామః", en: "whom after killing we do not wish to live", te: "ఎవరిని చంపి మనం బ్రతకాలని కోరుకోవడం లేదో" },
            { sanskrit: "te ’vasthitāḥ pramukhe dhārtarāṣṭrāḥ", sanskrit_te: "తేఽవస్థితాః ప్రముఖే ధార్తరాష్ట్రాః", en: "they stand before us, the sons of Dhritarashtra", te: "వారు మన ముందు నిలబడి ఉన్నారు, ధృతరాష్ట్ర పుత్రులు" }
        ]
    },
    "1.5": {
        exercises: {
            seeds: { question: "ఎంత మంది ధైర్యవంతులైన నాయకులు ఉన్నారా?", options: ["Yes, many! (🌟)", "No, only one (☝️)", "Zero (❌)"], correct: 0 },
            seekers: { question: "What quality is emphasized for these kings?", question_te: "ఈ రాజుల ఏ గొప్ప గుణాన్ని ఇక్కడ చెప్పారు?", options: ["They are rich", "They are powerful and heroic (Viryavan)", "They are funny"], correct: 1 },
            warriors: { question: "Why does Duryodhana list these names individually?", question_te: "దుర్యోధనుడు ఈ పేర్లన్నీ విడివిడిగా ఎందుకు చెప్పాడు?", options: ["He was bored", "To acknowledge the formidable strength of the opposition's alliances", "He liked the names"], correct: 1 }
        },
        telugu_script: "ధృష్టకేతుశ్చేకితానః కాశిరాజశ్చ వీర్యవాన్ |\nపురుజిత్కుంతిభోజశ్చ శైబ్యశ్చ నరపుంగవః ||",
        lineBreakdown: [
            { sanskrit: "dhṛṣṭaketuś cekitānaḥ", sanskrit_te: "ధృష్టకేతుశ్చేకితానః", en: "Dhrishtaketu and Chekitana", te: "ధృష్టకేతువు మరియు చేకితానుడు" },
            { sanskrit: "kāśirājaś ca vīryavān", sanskrit_te: "కాశిరాజశ్చ వీర్యవాన్", en: "and the heroic King of Kashi", te: "మరియు వీర్యవంతుడైన కాశీరాజు" },
            { sanskrit: "purujit kuntibhojaś ca", sanskrit_te: "పురుజిత్కుంతిభోజశ్చ", en: "Purujit and Kuntibhoja", te: "పురుజిత్తు మరియు కుంతిభోజుడు" },
            { sanskrit: "śaibyaś ca nara-puṅgavaḥ", sanskrit_te: "శైబ్యశ్చ నరపుంగవః", en: "and Saibya, the best of men", te: "మరియు శైబ్యుడు, నరశ్రేష్టుడు" }
        ]
    }
};

const CHAPTER_2_SHLOKAS = {
    "2.1": {
        sanskrit: "सञ्जय उवाच।\nतं तथा कृपयाविष्टमश्रुपूर्णाकुलेक्षणम्।\nविषीदन्तमिदं वाक्यमुवाच मधुसूदनः॥",
        transliteration: "sañjaya uvāca\ntaṁ tathā kṛpayāviṣṭam aśrupūrṇākulekṣaṇam\nviṣīdantam idaṁ vākyam uvāca madhusūdanaḥ",
        en: {
            meaning: "Sanjaya said: Seeing Arjuna full of compassion, his mind depressed, his eyes full of tears, Krishna spoke these words.",
            childMeaning: "Arjuna was very sad. Seeing this, his best friend Krishna began to speak to him to give him strength.",
            activity: "Draw a heart and write 'Stay Strong' inside it."
        },
        te: {
            meaning: "సంజయుడు ఇలా అన్నాడు: కరుణతో నిండి, కన్నీళ్లు పెట్టుకుంటూ విచారిస్తున్న అర్జునుడిని చూసి, కృష్ణుడు ఈ మాటలు పలికాడు.",
            childMeaning: "అర్జునుడు చాలా బాధలో ఉన్నాడు. అది చూసిన తన ప్రాణ స్నేహితుడు కృష్ణుడు అతనికి ధైర్యం చెప్పడం మొదలుపెట్టాడు.",
            activity: "ఒక గుండె బొమ్మ గీసి అందులో 'ధైర్యంగా ఉండు' అని రాయండి."
        },
        exercises: {
            seeds: { question: "How was Arjuna feeling?", question_te: "అర్జునుడు ఎలా ఉన్నాడు?", options: ["Happy (😊)", "Sad (😢)", "Angry (😡)"], correct: 1 },
            seekers: { question: "What did Krishna do?", question_te: "కృష్ణుడు ఏమి చేశాడు?", options: ["Ignored him", "Gave him courage", "Cried too"], correct: 1 },
            warriors: { question: "Meaning of 'Madhusudana' here?", question_te: "ఇక్కడ 'మధుసూదన' అంటే ఏమిటి?", options: ["Slayer of demon Madhu/doubts", "Lover of honey", "Archer"], correct: 0 }
        },
        telugu_script: "సంజయ ఉవాచ |\nతం తథా కృపయావిష్టమశ్రుపూర్ణాకులేక్షణమ్ |\nవిషీదంతమిదం వాక్యమువాచ మధుసూదనః ||",
        lineBreakdown: [
            { sanskrit: "taṁ tathā kṛpayāviṣṭam", sanskrit_te: "తం తథా కృపయావిష్టమ్", en: "to him who was overwhelmed by compassion", te: "కరుణతో నిండి ఉన్న అతనికి" },
            { sanskrit: "aśrupūrṇākulekṣaṇam", sanskrit_te: "అశ్రుపూర్ణాకులేక్షణమ్", en: "with eyes full of tears", te: "కన్నీళ్లతో నిండిన కళ్లతో" },
            { sanskrit: "viṣīdantam idaṁ vākyam", sanskrit_te: "విషీదంతమిదం వాక్యం", en: "lamenting, these words", te: "విచారిస్తున్న అతనికి ఈ మాటలు" },
            { sanskrit: "uvāca madhusūdanaḥ", sanskrit_te: "ఉవాచ మధుసూదనః", en: "spoke Madhusudana (Krishna)", te: "కృష్ణుడు పలికాడు" }
        ]
    },
    "2.2": {
        sanskrit: "श्रीभगवानुवाच।\nकुतस्त्वा कश्मलमिदं विषमे समुपस्थितम्।\nअनार्यजुष्टमस्वर्ग्यमकीर्तिकरमर्जुन॥",
        transliteration: "śrī-bhagavān uvāca\nkutas tvā kaśmalam idaṁ viṣame samupasthitam\nanārya-juṣṭam asvargyam akīrti-karam arjuna",
        en: {
            meaning: "Krishna said: My dear Arjuna, how have these impurities come upon you? They are not at all befitting a brave man.",
            childMeaning: "Krishna asked: 'Why are you feeling so weak? You are a brave warrior! These sad thoughts don't help you grow.'",
            activity: "What is one 'brave word' you can say to yourself?"
        },
        te: {
            meaning: "శ్రీకృష్ణుడు ఇలా అన్నాడు: అర్జునా, ఈ క్లిష్ట సమయంలో నీకు ఈ బలహీనత ఎక్కడి నుండి వచ్చింది? ఇది వీరులకు తగినది కాదు.",
            childMeaning: "కృష్ణుడు అడిగాడు: 'నువ్వు ఎందుకు ఇంత బలహీనంగా ఉన్నావు? నువ్వు ఒక వీరుడివి! ఈ బాధాకరమైన ఆలోచనలు నీ ఎదుగుదలకు పనికిరావు.'",
            activity: "నీకు నువ్వు చెప్పుకోగలిగే ఒక 'ధైర్యమైన మాట' ఏమిటి?"
        },
        exercises: {
            seeds: { question: "Krishna wants Arjuna to be...", question_te: "కృష్ణుడు అర్జునుడిని ఎలా ఉండమన్నాడు?", options: ["Brave (🦁)", "Scared (😨)", "Lazy (😴)"], correct: 0 },
            seekers: { question: "Why was Arjuna's sadness 'unfitting'?", question_te: "అర్జునుడి బాధ ఎందుకు తగనిది?", options: ["He was a hero", "He forgot lunch", "It was raining"], correct: 0 },
            warriors: { question: "Meaning of 'Anarya-justam'?", question_te: "'అనార్యజుష్టం' అంటే ఏమిటి?", options: ["Noble behavior", "Ignoble behavior", "Traveler"], correct: 1 }
        },
        telugu_script: "శ్రీభగవానువాచ |\nకుతస్త్వా కశ్మలమిదం విషమే సముపస్థితమ్ |\nఅనార్యజుష్టమస్వర్గ్యమకీర్తికరమర్జున ||",
        lineBreakdown: [
            { sanskrit: "kutas tvā kaśmalam idaṁ", sanskrit_te: "కుతస్త్వా కశ్మలమిదం", en: "how have these impurities come upon you", te: "నీకు ఈ మోహం ఎక్కడ నుండి వచ్చింది" },
            { sanskrit: "viṣame samupasthitam", sanskrit_te: "విషమే సముపస్థితమ్", en: "in this hour of crisis", te: "ఈ క్లిష్ట సమయంలో" },
            { sanskrit: "anārya-juṣṭam asvargyam", sanskrit_te: "అనార్యజుష్టమస్వర్గ్యమ్", en: "unfitting for a noble person; leading not to heaven", te: "ఆర్యజనులకు తగనిది; స్వర్గాన్ని ఇచ్చేది కాదు" },
            { sanskrit: "akīrti-karam arjuna", sanskrit_te: "అకీర్తికరమర్జున", en: "leading to infamy, O Arjuna", te: "అపకీర్తిని కలిగించేది, ఓ అర్జునా" }
        ]
    },
    "2.3": {
        sanskrit: "क्लैब्यं मा स्म गमः पार्थ नैतत्त्वय्युपपద్యते।\nक्षुద్రं हृदयदौర్बल्यं त्यक्त्वोत्तिष्ठ परन्तप॥",
        transliteration: "klaibyaṁ mā sma gamaḥ pārtha naitat tvayy upapadyate\nkṣudraṁ hṛdaya-daurbalyaṁ tyaktvottiṣṭha parantapa",
        en: {
            meaning: "Do not yield to this weakness. It does not befit you. Shake off this small worry and arise!",
            childMeaning: "Krishna said: 'Don't be a coward! Shake off this small worry and stand up ready to do your duty!'",
            activity: "Imagine Krishna telling you to 'Stand Up and Arise!' How do you feel?"
        },
        te: {
            meaning: "అర్జునా, బలహీనతకు లోనుకావద్దు, ఇది నీకు తగదు. నీ హృదయంలోని ఈ చిన్న బలహీనతను వదిలి పెట్టి, యుద్ధానికి సిద్ధంగా నిలబడు!",
            childMeaning: "కృష్ణుడు ఇలా అన్నాడు: 'భయపడవద్దు! నీ చిన్న చిన్న బాధలను వదిలేసి, నీ బాధ్యతను నెరవేర్చడానికి ధైర్యంగా నిలబడు!'",
            activity: "కృష్ణుడు నీతో 'లేచి నిలబడు!' అని చెబుతున్నట్లు ఊహించుకోండి. మీకు ఎలా అనిపిస్తుంది?"
        },
        exercises: {
            seeds: { question: "What should Arjuna do with worries?", question_te: "బాధల విషయంలో అర్జునుడు ఏమి చేయాలి?", options: ["Keep them", "Throw them away and stand up", "Sleep"], correct: 1 },
            seekers: { question: "Krishna calls worry what?", question_te: "బాధను కృష్ణుడు ఏమని పిలిచాడు?", options: ["Important", "Petty weakness", "Funny"], correct: 1 },
            warriors: { question: "Meaning of 'Parantapa'?", question_te: "'పరంతప' అంటే ఏమిటి?", options: ["Lazy", "Chastiser of enemies", "Father's name"], correct: 1 }
        },
        telugu_script: "క్లైబ్యం మా స్మ గమః పార్థ నైతత్త్వయ్యుపపద్యతే |\nక్షుద్రం హృదయదౌర్బల్యం త్యక్త్వోత్తిష్ఠ పరంతప ||",
        lineBreakdown: [
            { sanskrit: "klaibyaṁ mā sma gamaḥ pārtha", sanskrit_te: "క్లైబ్యం మా స్మ గమః పార్థ", en: "do not yield to impotence, O son of Pritha", te: "బలహీనతను పొందకు, ఓ అర్జునా" },
            { sanskrit: "naitat tvayy upapadyate", sanskrit_te: "నైతత్త్వయ్యుపపద్యతే", en: "it does not befit you", te: "ఇది నీకు తగదు" },
            { sanskrit: "kṣudraṁ hṛdaya-daurbalyaṁ", sanskrit_te: "క్షుద్రం హృదయదౌర్బల్యం", en: "petty weakness of heart", te: "తుచ్ఛమైన హృదయ దౌర్బల్యం" },
            { sanskrit: "tyaktvottiṣṭha parantapa", sanskrit_te: "త్యక్త్వోత్తిష్ఠ పరంతప", en: "giving up, arise, O chastiser of the enemy", te: "వీడి, లేచి నిలబడు ఓ పరంతపా" }
        ]
    },
    "2.4": {
        sanskrit: "अर्जुन उवाच।\nकथं भीष्ममहं सङ्ख्ये द्रोणं च मधुसूदन।\nइषुभिः प्रतियोत्स्यामि पूजार्हावरिसूदन॥",
        transliteration: "arjuna uvāca\nkathaṁ bhīṣmam ahaṁ saṅkhye droṇaṁ ca madhusūdana\niṣubhiḥ pratiyotsyāmi pూjārhāv arisūdana",
        en: {
            meaning: "Arjuna said: How can I fight against Bhishma and Drona, who are worthy of my worship?",
            childMeaning: "Arjuna said: 'How can I fight my grandfather and my teacher? I love them so much!'",
            activity: "Ever had a difficult choice between two things you love?"
        },
        te: {
            meaning: "అర్జునుడు ఇలా అన్నాడు: ఓ కృష్ణా, నేను పూజించదగిన భీష్ముడిని మరియు ద్రోణుడిని యుద్ధంలో బాణాలతో ఎలా ఎదిరించగలను?",
            childMeaning: "అర్జునుడు అన్నాడు: 'నేను నా తాతయ్యను, నా గురువును ఎలా ఎదిరించగలను? నేను వారిని ఎంతో ప్రేమిస్తున్నాను!'",
            activity: "నీకు నచ్చిన రెండు విషయాలలో ఒకదాన్ని ఎంచుకోవాల్సి వచ్చినప్పుడు ఎలా అనిపిస్తుంది?"
        },
        exercises: {
            seeds: { question: "Who did Arjuna love and respect?", question_te: "అర్జునుడు ఎవరిని గౌరవించాడు?", options: ["Bhishma & Drona", "Strangers", "Monsters"], correct: 0 },
            seekers: { question: "Why was he hesitant?", question_te: "అర్జునుడు ఎందుకు వెనుకడుగు వేశాడు?", options: ["Scared", "Respected them", "No arrows"], correct: 1 },
            warriors: { question: "Primary conflict in 2.4?", question_te: "2.4 శ్లోకంలో ప్రధాన సంఘర్షణ ఏమిటి?", options: ["Fear of injury", "Duty vs respect for elders", "Waking up early"], correct: 1 }
        },
        telugu_script: "అర్జున ఉవాచ |\nకథం భీష్మమహం సంఖ్యే ద్రోణం చ మధుసూదన |\nఇషుభిః ప్రతియోత్స్యామి పూజార్హావరిసూదన ||",
        lineBreakdown: [
            { sanskrit: "kathaṁ bhīṣmam ahaṁ saṅkhye", sanskrit_te: "కథం భీష్మమహం సంఖ్యే", en: "how can I Bhishma in battle", te: "యుద్ధంలో భీష్ముడిని నేను ఎలా" },
            { sanskrit: "droṇaṁ ca madhusūdana", sanskrit_te: "ద్రోణం చ మధుసూదన", en: "and Drona, O slayer of Madhu", te: "మరియు ద్రోణుడిని, ఓ మధుసూదనా" },
            { sanskrit: "iṣubhiḥ pratiyotsyāmi", sanskrit_te: "ఇషుభిః ప్రతియోత్స్యామి", en: "fight back with arrows", te: "బాణాలతో ఎలా ఎదిరించగలను" },
            { sanskrit: "pūjārhāv arisūdana", sanskrit_te: "పూజార్హావరిసూదన", en: "who are worthy of worship", te: "పూజ్యులైన వారిని" }
        ]
    },
    "2.5": {
        sanskrit: "गुरूनहत्वा हि महानुभावान् श्रेयो भोक्तुं भैक्ष्यमपीह లోకే।\nहत्वार्थकामांस्तु गुरूनिहैव भुञ्जीय भोगान् रुधिरप्रदिग्धान्॥",
        transliteration: "gurūn ahatvā hi mahā-nubhāvān śreyo bhoktuṁ bhaikṣyam apīha loke\nhatvārtha-kāmāṁs tu gurūn ihaiva bhuñjīya bhogān rudhira-pradigdhān",
        en: {
            meaning: "It would be better to live in this world by begging than to enjoy life by killing such noble teachers.",
            childMeaning: "Arjuna said: 'I would rather have nothing and be a beggar than win by hurting my great teachers.'",
            activity: "What is more important: Winning a game, or being a kind friend?"
        },
        te: {
            meaning: "గొప్పవారైన గురువులను చంపడం కంటే, ఈ లోకంలో భిక్షాటన చేస్తూ బ్రతకడం మేలు. వారిని చంపి పొందే భోగాలు రక్తంతో తడిసినవే అవుతాయి.",
            childMeaning: "అర్జునుడు అన్నాడు: 'నా గొప్ప గురువులను బాధపెట్టి గెలవడం కంటే, నేనేమీ లేకుండా భిక్షగాడిగా ఉండటమే మేలు.'",
            activity: "ఆటలో గెలవడం ముఖ్యమా లేక మంచి స్నేహితుడిగా ఉండటం ముఖ్యమా?"
        },
        exercises: {
            seeds: { question: "What did Arjuna say was better than hurting his teachers?", question_te: "గురువులను బాధపెట్టడం కంటే ఏది మేలని అర్జునుడు అన్నాడు?", options: ["Being a beggar", "Winning gold", "Watching TV"], correct: 0 },
            seekers: { question: "Arjuna thought winning would feel bad because...", question_te: "గెలవడం ఎందుకు బాధాకరమని అర్జునుడు భావించాడు?", options: ["Trophy heavy", "Involve hurting noble people", "No like winning"], correct: 1 },
            warriors: { question: "Arjuna's character?", question_te: "అర్జునుడి స్వభావం ఎలాంటిది?", options: ["Greedy", "High ethical standards", "Lazy"], correct: 1 }
        },
        telugu_script: "గురూనహత్వా హి మహానుభావాన్ శ్రేయో భోక్తుం భైక్ష్యమపీహ లోకే |\nహత్వార్థకామాంస్తు గురూనిహైవ భుంజీయ భోగాన్ రుధిరప్రదిగ్ధాన్ ||",
        lineBreakdown: [
            { sanskrit: "gurūn ahatvā hi mahā-nubhāvān", sanskrit_te: "గురూనహత్వా హి మహానుభావాన్", en: "without killing the great teachers", te: "గొప్ప గురువులను చంపకుండా" },
            { sanskrit: "śreyo bhoktuṁ bhaikṣyam apīha loke", sanskrit_te: "శ్రేయో భోక్తుం భైక్ష్యమపీహ లోకే", en: "better to live by begging in this world", te: "ఈ లోకంలో భిక్షాటన చేస్తూ బ్రతకడం మేలు" },
            { sanskrit: "hatvārtha-kāmāṁs tu gurūn", sanskrit_te: "హత్వార్థకామాంస్తు గురూన్", en: "but killing gurus who desire wealth", te: "కానీ లాభం కోసం గురువులను చంపడం" },
            { sanskrit: "bhuñjīya bhogān rudhira-pradigdhān", sanskrit_te: "భుంజీయ భోగాన్ రుధిరప్రదిగ్ధాన్", en: "would be enjoying blood-stained pleasures", te: "రక్తంతో తడిసిన భోగాలను అనుభవించడమే" }
        ]
    },
    "2.7": {
        sanskrit: "कार्पण्यदोषोपहतस्वभावः पृच्छामि त्वां धर्मसम्मूढचेताः।\nयच्छ्रेयः स्यान्निश्चितं ब्रूहि तन्మే शिष्यस्तेऽहं शाधि मां त्वां प्रपन्नम्॥",
        transliteration: "kārpaṇya-doṣopahata-svabhāvaḥ pṛcchāmi tvāṁ dharma-sammūḍha-cetāḥ\nyac chreyaḥ syān niścitaṁ brūhi tan me śiṣyas te ’haṁ śādhi māṁ tvāṁ prapannam",
        en: {
            meaning: "I am confused about my duty. Tell me for certain what is best. I am Your disciple. Please instruct me.",
            childMeaning: "Arjuna said: 'I am so confused! Please be my teacher. I promise to listen. Tell me exactly what I should do.'",
            activity: "Think of one big question you want to ask a wise person."
        },
        te: {
            meaning: "కర్తవ్యం పట్ల నాకు సందేహం కలుగుతోంది. ఏది శ్రేయస్కరమో నాకు ఖచ్చితంగా చెప్పు. నేను నీ శిష్యుడిని, నన్ను శాసించు.",
            childMeaning: "అర్జునుడు అన్నాడు: 'నాకు ఏమీ అర్థం కావడం లేదు! ప్లీజ్ నాకు గురువుగా ఉండి దారి చూపించు. నువ్వు ఏం చెబితే అది వింటాను.'",
            activity: "మీకు ఏదైనా సందేహం ఉన్నప్పుడు ఎవరిని అడుగుతారు?"
        },
        exercises: {
            seeds: { question: "What did Arjuna ask Krishna to be?", question_te: "అర్జునుడు కృష్ణుడిని ఏమని వేడుకున్నాడు?", options: ["Teacher", "Driver", "Cook"], correct: 0 },
            seekers: { question: "Meaning of 'Shishyas te 'ham'?", question_te: "'శిష్యస్తేఽహం' అంటే ఏమిటి?", options: ["Friend", "Teacher", "Disciple"], correct: 2 },
            warriors: { question: "Why surrender?", question_te: "అర్జునుడు ఎందుకు కృష్ణుడికి శరణు కోరాడు?", options: ["Lazy", "Clouded logic", "Krishna fight"], correct: 1 }
        },
        telugu_script: "కార్పణ్యదోషోపహతస్వభావః పృచ్ఛామి త్వాం ధర్మసమ్మూఢచేతాః |\nయచ్ఛ్రేయః స్యాన్నిశ్చితం బ్రూహి తన్మే శిష్యస్తేఽహం శాధి మాం త్వాం ప్రపన్నమ్ ||",
        lineBreakdown: [
            { sanskrit: "kārpaṇya-doṣopahata-svabhāvaḥ", sanskrit_te: "కార్పణ్యదోషోపహతస్వభావః", en: "stricken by the weakness of pity", te: "కరుణ అనే బలహీనతతో నిండి" },
            { sanskrit: "pṛcchāmi tvāṁ dharma-sammūḍha-cetāḥ", sanskrit_te: "పృచ్ఛామి త్వాం ధర్మసమ్మూఢచేతాః", en: "I ask you, confused about duty", te: "ధర్మం పట్ల అయోమయంలో నిన్ను అడుగుతున్నాను" },
            { sanskrit: "yac chreyaḥ syān niścitaṁ brūhi", sanskrit_te: "యచ్ఛ్రేయః స్యాన్నిశ్చితం బ్రూహి", en: "tell me for certain what is best", te: "నాకు ఏది శ్రేయస్కరమో ఖచ్చితంగా చెప్పు" },
            { sanskrit: "śiṣyas te ’haṁ śādhi māṁ", sanskrit_te: "శిష్యస్తేఽహం శాధి మాం", en: "I am your disciple, please instruct me", te: "నేను నీ శిష్యుడిని, నాకు దారి చూపించు" }
        ]
    },
    "2.11": {
        sanskrit: "श्रीभगवानुवाच।\nअशोच्यानन्वशोचस्त्वं प्रज्ञावादांश्च भाषसे।\nगतासूनगतासूंश्च नानुशोचन्ति पण्डिताः॥",
        transliteration: "śrī-bhagavān uvāca\naśocyān anvaśocas tvaṁ prajñā-vādāṁś ca bhāṣase\ngatāsūn agatāsūṁś ca nānuśocanti paṇḍitāḥ",
        en: {
            meaning: "The Supreme Personality of Godhead said: Wise people do not worry about the living or the dead.",
            childMeaning: "Krishna said: 'You talk smart, but worry about small things! Wise people know the soul stays forever.'",
            activity: "Think of a happy memory with a lost toy."
        },
        te: {
            meaning: "శ్రీభగవానుడు ఇలా అన్నాడు: అర్జునా, నువ్వు విచారించకూడని వాటి కోసం విచారిస్తున్నావు. జ్ఞానులు జీవించి ఉన్నవారి గురించి కానీ, మరణించిన వారి గురించి కానీ దుఃఖించరు.",
            childMeaning: "కృష్ణుడు అన్నాడు: 'నువ్వు గొప్ప మాటలు మాట్లాడుతున్నావు, కానీ చిన్న విషయాలకే భయపడుతున్నావు! ప్రాణం ఎప్పటికీ ఉంటుందని తెలిసిన వారు బాధపడరు.'",
            activity: "నీకు బాగా నచ్చిన ఒక పాత జ్ఞాపకాన్ని గుర్తు చేసుకో."
        },
        exercises: {
            seeds: { question: "Do wise people worry all the time?", question_te: "జ్ఞానులు ఎప్పుడూ బాధపడుతుంటారా?", options: ["Yes", "No, stay calm", "Tuesdays"], correct: 1 },
            seekers: { question: "Krishna's view on Arjuna's worries?", question_te: "అర్జునుడి బాధ గురించి కృష్ణుడు ఏమన్నాడు?", options: ["Important", "Not worthy of grief", "Worry more"], correct: 1 },
            warriors: { question: "Meaning of 'Prajnavadams'?", question_te: "'ప్రజ్ఞావాదములు' అంటే ఏమిటి?", options: ["Genius", "Logic for weakness", "Orator"], correct: 1 }
        },
        telugu_script: "శ్రీభగవానువాచ |\nఅశోచ్యానన్వశోచస్త్వం ప్రజ్ఞావాదాంశ్చ భాషసే |\nగతాసూనగతాసూంశ్చ నానుశోచంతి పండితాః ||",
        lineBreakdown: [
            { sanskrit: "aśocyān anvaśocas tvaṁ", sanskrit_te: "అశోచ్యానన్వశోచస్త్వం", en: "grieving for what is not worthy of grief", te: "శోచించకూడని వాటి గురించి బాధపడుతున్నావు" },
            { sanskrit: "prajñā-vādāṁś ca bhāṣase", sanskrit_te: "ప్రజ్ఞావాదాంశ్చ భాషసే", en: "while speaking words of wisdom", te: "జ్ఞానయుక్తమైన మాటలు మాట్లాడుతున్నావు" },
            { sanskrit: "gatāsūn agatāsūṁś ca", sanskrit_te: "గతాసూనగతాసూంశ్చ", en: "for the dead or the living", te: "చనిపోయిన వారి గురించి లేదా ఉన్నవారి గురించి" },
            { sanskrit: "nānuśocanti paṇḍitāḥ", sanskrit_te: "నానుశోచంతి పండితాః", en: "the wise do not lament", te: "జ్ఞానులు దుఃఖించరు" }
        ]
    },
    "2.13": {
        sanskrit: "देहिनोऽस्मिन्यथा देहे कौमारं यौवनं जरा।\nतथा देहान्तरप्राप्तिर्धीरस्तत्र न मुह्यతి॥",
        transliteration: "dehino ’smin yathā dehe kaumāraṁ yauvanaṁ jarā\ntathā dehāntara-prāptir dhīras tatra na muhyati",
        en: {
            meaning: "As the soul passes from boyhood to youth to old age, it passes into another body at death.",
            childMeaning: "Just like you grow from a baby to a kid, the 'real you' (soul) just moves from one 'costume' to another.",
            activity: "Look at a baby photo of yourself."
        },
        te: {
            meaning: "ఈ దేహంలో ఉన్న జీవుడికి బాల్యం, యవ్వనం, ముసలితనం ఎలా కలుగుతాయో, మరణం తర్వాత మరో దేహాన్ని పొందడం కూడా అలాగే జరుగుతుంది.",
            childMeaning: "నువ్వు చిన్న పాప నుండి పెద్ద బాబుగా ఎలా మారుతావో, లోపల ఉన్న 'నువ్వు' (ఆత్మ) కూడా ఒక డ్రెస్ నుండి మరో డ్రెస్ మారినట్లు మారుతుంది.",
            activity: "నీ చిన్నప్పటి ఫోటోను ఒకసారి చూడు."
        },
        exercises: {
            seeds: { question: "Do you stay same inside as you grow?", question_te: "నువ్వు పెరుగుతున్నా లోపల ఉన్న 'నువ్వు' మారకుండా ఉంటావా?", options: ["Yes", "No, cat", "Maybe"], correct: 0 },
            seekers: { question: "Body change comparison?", question_te: "శరీరం మారడాన్ని దేనితో పోల్చారు?", options: ["Clothes", "Childhood to old age", "Bus"], correct: 1 },
            warriors: { question: "Soul lesson?", question_te: "ఆత్మ గురించి ఇక్కడ ఏం చెప్పారు?", options: ["Grows bigger", "Remains unchanged", "Disappears"], correct: 1 }
        },
        telugu_script: "దేహినోఽస్మిన్యథా దేహే కౌమారం యౌవనం జరా |\nతథా దేహాంతరప్రాప్తిర్ధీరస్తత్ర న ముహ్యతి ||",
        lineBreakdown: [
            { sanskrit: "dehino ’smin yathā dehe", sanskrit_te: "దేహినోఽస్మిన్యథా దేహే", en: "as in this body of the embodied", te: "ఈ దేహంలో ఉన్న జీవుడికి" },
            { sanskrit: "kaumāraṁ yauvanaṁ jarā", sanskrit_te: "కౌమారం యౌవనం జరా", en: "boyhood, youth, and old age", te: "బాల్యం, యవ్వనం మరియు ముసలితనం" },
            { sanskrit: "tathā dehāntara-prāptir", sanskrit_te: "తథా దేహాంతరప్రాప్తిర్", en: "similarly, attaining another body", te: "అలాగే మరో దేహాన్ని పొందడం" },
            { sanskrit: "dhīras tatra na muhyati", sanskrit_te: "ధీరస్తత్ర న ముహ్యతి", en: "a wise person is not confused", te: "జ్ఞాని దీనికి కలత చెందడు" }
        ]
    },
    "2.20": {
        sanskrit: "न जायते म्रियते वा कदाचिन्नायं भूत्वा भविता वा न भूयः।\nअजो नित्यः शाश्वतोऽयं पुराणो न हन्यते हन्यमाने शरीरे॥",
        transliteration: "na jāyate mriyate vā kadācin nāyaṁ bhūtvā bhavitā vā na bhūyaḥ\najo nityaḥ śāśvato ’yaṁ purāṇo na hanyate hanyamāne śarīre",
        en: {
            meaning: "For the soul there is neither birth nor death. He is unborn, eternal and ever-existing.",
            childMeaning: "The soul is like a superhero that can never be hurt! It was never born and never dies.",
            activity: "What 'Super Power' do you wish you had?"
        },
        te: {
            meaning: "ఆత్మకు ఎప్పుడూ పుట్టుక కానీ, మరణం కానీ ఉండవు. అది పుట్టనిది, నిత్యమైనది, శాశ్వతమైనది. శరీరం నశించినా అది నశించదు.",
            childMeaning: "నీ ఆత్మ ఒక సూపర్ హీరో లాంటిది! దానికి దెబ్బ తగలదు, అది ఎప్పుడూ చనిపోదు.",
            activity: "నీకు ఏ 'సూపర్ పవర్' ఉండాలని కోరుకుంటావు?"
        },
        exercises: {
            seeds: { question: "Can the soul be hurt?", question_te: "ఆత్మకు దెబ్బ తగులుతుందా?", options: ["Yes", "No, super strong", "Maybe"], correct: 1 },
            seekers: { question: "Is the soul born with the body?", question_te: "శరీరంతో పాటు ఆత్మ కూడా పుడుతుందా?", options: ["Yes", "No, unborn", "Depends"], correct: 1 },
            warriors: { question: "Soul vs body at death?", question_te: "మరణం తర్వాత ఆత్మ ఏమవుతుంది?", options: ["Both destroyed", "Soul remains unaffected", "Disappears"], correct: 1 }
        },
        telugu_script: "న జాయతే మ్రియతే వా కదాచిన్నాయం భూత్వా భవితా వా న భూయః |\nఅజో నిత్యః శాశ్వతోఽయం పురాణో న హన్యతే హన్యమానే శరీరే ||",
        lineBreakdown: [
            { sanskrit: "na jāyate mriyate vā kadācin", sanskrit_te: "న జాయతే మ్రియతే వా కదాచిత్", en: "is never born nor dies at any time", te: "పుట్టుక మరియు మరణం లేనిది" },
            { sanskrit: "ajo nityaḥ śāśvato ’yaṁ", sanskrit_te: "అజో నిత్యః శాశ్వతోఽయం", en: "unborn, eternal, ever-existing", te: "అజన్ముడు, నిత్యుడు, శాశ్వతుడు" },
            { sanskrit: "na hanyate hanyamāne śarīre", sanskrit_te: "న హన్యతే హన్యమానే శరీరే", en: "is not slain when the body is slain", te: "శరీరం నశించినా ఇది నశించదు" }
        ]
    },
    "2.47": {
        sanskrit: "कर्मण्येवाधिकारस्ते मा फलेषु कदाचन।\nमा कर्मफलहेतुर्भూర్మా తే సఙ్గోऽస్త్వకర్మణి॥",
        transliteration: "karmaṇy-evādhikāras te mā phaleṣu kadācana\nmā karma-phala-hetur bhūr mā te saṅgo ’stv akarmaṇi",
        en: {
            meaning: "Focus on your duty, not the result. Don't be attached to inaction.",
            childMeaning: "Focus on effort and enjoy the activity! The fun is in the doing, not just the prize.",
            activity: "Practice a hobby for 20 minutes today."
        },
        te: {
            meaning: "కర్తవ్యాన్ని నిర్వహించు, ఫలితాన్ని ఆశించకు. పని చేయకుండా సోమరిగా ఉండవద్దు.",
            childMeaning: "నీ వంతు ప్రయత్నం నువ్వు చేయి, ఫలితం గురించి భయం వద్దు. చేసే పనిని ఇష్టపడి చేయి!",
            activity: "ఈ రోజు నీకు నచ్చిన పనిని 20 నిమిషాలు ఏకాగ్రతతో చేయి."
        },
        exercises: {
            seeds: { question: "Most important in a task?", question_te: "ఒక పనిలో అత్యంత ముఖ్యమైనది ఏమిటి?", options: ["Reward", "Doing best with joy", "Speed"], correct: 1 },
            seekers: { question: "What to focus on?", question_te: "మనం దేని మీద దృష్టి పెట్టాలి?", options: ["Winning", "Effort and joy", "Others"], correct: 1 },
            warriors: { question: "Control lesson?", question_te: "మనం దేనిని నియంత్రించగలము?", options: ["Control outcomes", "Control actions, not results", "Do nothing"], correct: 1 }
        },
        telugu_script: "కర్మణ్యేవాధికారస్తే మా ఫలేషు కదాచన |\nమా కర్మఫలహేతుర్భూర్మా తే సఙ్గోऽస్త్వకర్మణి ||",
        lineBreakdown: [
            { sanskrit: "Karmani eva adhikarah te", sanskrit_te: "కర్మణ్యేవాధికారస్తే", en: "You have the right only to perform your duties", te: "కర్మను చేయుటకే నీకు అధికారము కలదు" },
            { sanskrit: "Ma phaleshu kadachana", sanskrit_te: "మా ఫలేషు కదాచన", en: "Do not be attached to the results", te: "ఫలితములపై ఎన్నడూ ఆసక్తి చూపకు" },
            { sanskrit: "Ma karma-phala-hetuh bhur", sanskrit_te: "మా కర్మఫలహేతుర్భూర్", en: "Do not act just for reward", te: "కర్మఫలమునకు కారణము కావద్దు" },
            { sanskrit: "Ma te sango’stvakarmani", sanskrit_te: "మా తే సఙ్గోऽస్త్వకర్మణి", en: "Do not cling to inaction", te: "పని చేయకుండా ఉండటంపై ఆసక్తి పెంచుకోవద్దు" }
        ]
    }
};

window.GITA_SHLOKAS = { ...CHAPTER_1_SHLOKAS, ...CHAPTER_2_SHLOKAS };
