...
function normalizeVerseEntry(entry) {
  ...
  const meaningsEn = (entry.meaningsEn || entry.enMeanings || [])
    .map((s) => String(s).trim())
    .filter(Boolean);
  const meaningsTe = (entry.meaningsTe || entry.teMeanings || [])
    .map((s) => String(s).trim())
    .filter(Boolean);
  ...
  return {
    ...(intro ? { intro } : {}),
    lines,
    ...(meaningsEn.length ? { meaningsEn } : {}),
    ...(meaningsTe.length ? { meaningsTe } : {}),
  };
}
