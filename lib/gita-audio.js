...
/** Fresh signed HF URL (Vercel FS is read-only — do not rely on saving manifest at runtime). */
...
async function resolveHfPlayUrl(verseId) {
  ...
  if (trainIndex != null) {
    const fresh = await fetchHfAudioUrlByTrainIndex(trainIndex);
...
function registerGitaAudioRoutes(router) {
  router.get('/manifest', (req, res) => {
    ...
    res.json({
      ...
      verses: verses.reduce((acc, id) => {
        ...
        const proxyUrl = `/api/gita-audio/${encodeURIComponent(id)}`;
        const entry = {
          url: proxyUrl,
  ...
  router.get('/:verseId', async (req, res) => {
    try {
      ...
      const hfUrl = await resolveHfPlayUrl(verseId);
      if (hfUrl) {
        const hfRes = await fetch(hfUrl);
        if (!hfRes.ok) {
          console.warn(`[gita-audio] HF ${hfRes.status} for ${verseId}`);
          return res.status(502).json({ error: 'Chant audio upstream failed', verseId });
        }
        const buf = Buffer.from(await hfRes.arrayBuffer());
        res.setHeader('Content-Type', hfRes.headers.get('content-type') || 'audio/wav');
        res.setHeader('Cache-Control', 'public, max-age=3600');
        return res.send(buf);
      }
      return res.status(404).json({ error: 'Chant audio not available', verseId });
    } catch (err) {
      console.error('[gita-audio]', err.message);
      return res.status(502).json({ error: 'Chant audio resolve failed' });
    }
  });
}
